#+title: Easy mode encryption for orgmode with age.el
...
(defun bhankas-org-age-encrypt-and-replace ()
  ...
  (let* ((current-file-name (buffer-file-name))
         (encr-file-name (-> (buffer-file-name)
                             (string-trim)
                             (concat ".age"))))
    (if (string-suffix-p ".org" current-file-name)
        (progn
...
This takes care of dependency and loading order.[fn:4]

However, this still requires manually doing =M-x bhankas-org-age-encrypt-and-replace=. That is fine and desired, even, for existing nodes, but I wanted further automation of notes being encrypted. 100% of my org documents are created via =org-capture=[fn:5], so the natural place and time to encrypt notes are right when they are created. But, I also wanted this to happen for only specific capture templates[fn:6].

With bit of fiddling[fn:7], this is also done:
...
[fn:4] Please note that =use-package!= macro comes from fantastic =doom-emacs=. Non-doom-users can simply replace it with =use-package= which is included in Emacs 29+.

[fn:5] Or its cousin [[https://www.orgroam.com/manual.html#The-Templating-System][org-roam-capture]].

[fn:6] like daily notes and some specific, personal notes, but not the generic topic based notes, those can be public and I want to be able to access them as plaintext docs.
