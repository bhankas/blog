#+title: Modifying emacs-lisp function arguments conditionally
...
But, the pop-uup buffer automatically takes as much space as necessary by default. This is a problem, because for repeated tasks (e.g. watering plants) the LOGBOOK property drawer can be hundreds of lines, and the pop-up occupies nearly entire display, sort-of defeating the purpose.

So, I went about trying to find what the =org-agenda= keybinds are doing. This is where the rabbit hole leads:
...
I can simply add an extra argument for ~MAX-HEIGHT~ and it will do what I want.

But, it turns out, this =fit-window-to-buffer= function is called quite often, most notably by the awesome =which-key=. So I want to modify the argument list, but only when it is called by the aforenoticed =org-agenda-tree-indirect-buffer=.

It turns out emacs-lisp has some nice functionality built-int to walk the stack-trace (or in elisp ling, a =backtrace=). So I cooked up a function that walks the backtrace to determine if =fit-window-to-buffer= was called by =org-agenda-tree-indirect-buffer= and append ~MAX-HEIGHT~ to arguments list if it is:
